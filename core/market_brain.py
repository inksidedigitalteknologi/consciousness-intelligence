...
def analyze_market_full(
    ...
    try:
        ...
        if global_brain is not None:
            ...
            # === Filter last_decision: hanya tampilkan jika untuk symbol ini ===
            last_decision = {}
            last_input = last.get('input', {}) if isinstance(last, dict) else {}
            last_symbol = None
            if isinstance(last_input, dict):
                last_symbol = last_input.get('symbol')
            # Fallback: cek dari field 'input_data' di dalam 'input' string
            if not last_symbol and isinstance(last_input, str):
                # input bisa berupa repr CognitiveState — coba regex symbol
                import re as _re
                m = _re.search(r"'symbol':\s*'([^']+)'", last_input)
                if m:
                    last_symbol = m.group(1)

            symbol_matches = (
                last_symbol is not None
                and str(last_symbol).upper() == symbol.upper()
            )

            if symbol_matches:
                last_decision = (
                    last.get('decision')
                    if isinstance(last.get('decision'), dict)
                    else {}
                )
            ...
            brain_output = {
                ...
                'last_decision_symbol': last_symbol,
                'last_decision_matches': symbol_matches,
    ...
    try:
        ...
        if current_price:
            if action == 'BUY':
                entry = round(current_price, 2)  # entry di harga sekarang
                ...
                # Target dari resistance
                target_resistance = round(resistance * 0.99, 2) if resistance else round(entry * 1.05, 2)
                
                # Target minimal 2x risk (risk/reward 1:2)
                target_min_rr = round(entry + (risk * 2), 2)
                
                # Ambil yang lebih tinggi
                target = max(target_resistance, target_min_rr)
                
                # Pastikan target > entry
                if target <= entry:
                ...
                # Target dari support
                target_support = round(support * 1.01, 2) if support else round(entry * 0.95, 2)
                
                # Target minimal 2x risk (risk/reward 1:2)
                target_min_rr = round(entry - (risk * 2), 2)
                
                # Ambil yang lebih rendah
                target = min(target_support, target_min_rr)
        ...
        try:
            ...
            logger.debug(f"analyze_unified error: {_ue}")
            unified = {'error': str(_ue)}
    ...
    # ============================================================
    # 3c. FEED KE BRAIN — observe hasil analisis
    # ============================================================
    try:
        from core.brain import brain as _global_brain
        if _global_brain is not None and isinstance(market_decision, dict):
            _unified = market_decision.get('unified', {})
            if isinstance(_unified, dict) and 'score' in _unified:
                _global_brain.observe({
                    'symbol': symbol,
                    'action': _unified.get('action'),
                    'score': _unified.get('score'),
                    'confidence': _unified.get('confidence'),
                    'aspek_count': _unified.get('aspek_count', 0),
                    'breakdown_summary': {
                        k: v for k, v in list((_unified.get('breakdown') or {}).items())[:5]
                    },
                })
