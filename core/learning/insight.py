...
class InsightEngine:
    ...
    def generate(
        self, data: Any, source: str = "unknown", metadata: Optional[Dict[str, Any]] = None
    ...
    def create_summary(self, data: Dict[str, Any]) -> str:
        ...
        # Prediction
        predicted = (
            prediction.get("prediction") or prediction.get("signal") or prediction.get("direction")
        )
        if predicted:
        ...
        for source_name, item in sources:
            ...
            for key, value in item.items():
                if value is not None:
                    points.append({"source": source_name, "factor": str(key), "value": value})
    ...
    def calculate_confidence(self, data: Dict[str, Any]) -> float:
        ...
        for source in sources:
            ...
            value = (
                source.get("confidence") or source.get("score") or source.get("confidence_score")
            )
            if value is None:
    ...
    def detect_conflicts(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Extract signals
        analysis_signal = self._normalize_signal(analysis.get("signal") or analysis.get("trend"))
        prediction_signal = self._normalize_signal(
            prediction.get("signal") or prediction.get("direction") or prediction.get("prediction")
        )
        decision_signal = self._normalize_signal(decision.get("action") or decision.get("signal"))
        reasoning_signal = self._normalize_signal(
            reasoning.get("conclusion") or reasoning.get("direction")
        )
        strategy_signal = self._normalize_signal(strategy.get("bias") or strategy.get("direction"))
    ...
    def calculate_importance(
        self, data: Dict[str, Any], confidence: float = 0.0, conflicts: Optional[List[Dict]] = None
    ) -> float:
    ...
    def compare(self, old: Optional[Dict], new: Optional[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_change": (
                (self._numeric(new_confidence) or 0) - (self._numeric(old_confidence) or 0)
            ),
            "improved": (
                (self._numeric(new_confidence) or 0) > (self._numeric(old_confidence) or 0)
            ),
    ...
    def statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_confidence": round(
                sum(item.get("confidence", 0) for item in self.insights)
                / max(1, len(self.insights)),
                2,
            ),
        }
...
# ============================================================
# core/brain.py imports: safe_import("core.learning.insight", "insight")
