...
class KnowledgeBuilder:
    ...
    def __init__(self, max_knowledge: int = 1000, config: Optional[Dict[str, Any]] = None):
    ...
    def build(
        self, experience: Any, source: str = "unknown", metadata: Optional[Dict[str, Any]] = None
        ...
        try:
            ...
            confidence = self.calculate_confidence(experience, concepts, facts, patterns)
    ...
    def calculate_confidence(
        self, experience: Any, concepts: List[str], facts: List[Dict], patterns: List[Dict]
    ) -> float:
    ...
    def related(self, concept: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        ordered = sorted(relations.items(), key=lambda item: item[1], reverse=True)

        return [{"concept": item[0], "strength": item[1]} for item in ordered[: max(1, int(limit))]]
    ...
    def deprecate(self, knowledge_id: str, reason: str = "") -> bool:
        ...
        for item in self.knowledge:
            if item.get("id") == knowledge_id:
                ...
                item["history"].append(
                    {"action": "deprecated", "reason": reason, "timestamp": utc_now()}
                )
                return True
        return False
    ...
    def statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": round(
                sum(k.get("confidence", 0) for k in self.knowledge) / max(1, len(self.knowledge)), 2
            ),
