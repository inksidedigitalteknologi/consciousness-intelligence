...
class KnowledgeEngine:
        ...
        try:
            with self.lock:
                ...
                for attempt in range(3):
                    ...
                    raise last_error if last_error else Exception("Save failed after 3 attempts")
    ...
    def classify_content(self, content: str) -> str:
        """AI auto-classify content menjadi category."""
        try:
            from core.deepseek import deepseek_ai
            if not deepseek_ai or not getattr(deepseek_ai, 'enabled', False):
                return KnowledgeCategory.GENERAL.value

            prompt = (
                f"Klasifikasikan konten ini ke SALAH SATU kategori berikut:\n"
                f"- Trading\n- Market\n- Finance\n- Strategy\n- Bitcoin\n"
                f"- Crypto\n- AI\n- Economy\n- Education\n- News\n- General\n\n"
                f"OUTPUT: HANYA nama kategori, tanpa penjelasan, tanpa tanda baca.\n\n"
                f"Konten:\n{content[:500]}"
            )
            ...
            valid = ['Trading', 'Market', 'Finance', 'Strategy', 'Bitcoin',
                     'Crypto', 'AI', 'Economy', 'Education', 'News', 'General']

            for v in valid:
                if v.lower() in cleaned.lower():
                    logger.info(f"🤖 Auto-classified as: {v}")
                    return v

            logger.info(f"🤖 Auto-classify fallback ke General (raw: {cleaned[:30]})")
            return KnowledgeCategory.GENERAL.value
        except Exception as e:
    ...
    def add(
        ...
        # Auto-classify kalau category kosong / "Auto" / "General Knowledge" default
        if not category or category in ("Auto", "General Knowledge", "General"):
    ...
    def _enhance_with_ai(self, item_id: str) -> bool:
        ...
        try:
            ...
            if not item.ai_summary:
                try:
                    item.ai_summary = deepseek_ai._summarize(item.content)
                except:
            ...
            if not item.ai_insights:
                try:
                    item.ai_insights = deepseek_ai._extract_insights(item.content)
                except:
            ...
            if not item.ai_tags:
                try:
                    item.ai_tags = deepseek_ai._generate_tags(item.content)
                except:
