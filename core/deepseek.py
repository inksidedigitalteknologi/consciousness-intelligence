...
class DeepSeekAI:
    ...
    def ask(
        ...
        # 3. Check cache
        import hashlib
        # Cache key pakai hash penuh dari question + context (normalized)
        _q_norm = question.strip().lower()
        _c_norm = (context or '').strip().lower()
        cache_key = hashlib.md5(f"{_q_norm}||{_c_norm}".encode()).hexdigest()
        if use_cache:
    ...
    def _generate_tags(self, content: str) -> List[str]:
        ...
        try:
            prompt = (
                f"Buat 3-5 tag (1-2 kata, lowercase) untuk konten ini.\n\n"
                f"OUTPUT: HANYA daftar tag, SATU PER BARIS, tanpa nomor, tanpa bullet, tanpa penjelasan.\n\n"
                f"Contoh output BENAR:\ninterstellar\nastronomi\nsains\n\n"
                f"Konten:\n{content[:500]}"
            )
            ...
            for line in result.split(chr(10)):
                ...
                if not line or len(line) > 30:
                    continue
                if any(k in line_lower for k in ["tag untuk", "berikut", "contoh", "catatan", "daftar", "output", "konten", "tersebut"]):
                    continue
                if line.count(",") >= 2:
                    continue
                cleaned = line_lower.replace(" ", "-").strip("-")
                if cleaned and 2 < len(cleaned) < 30:
                    tags.append(cleaned)

            if not tags and "," in result:
                for word in result.split(","):
                    cleaned = word.strip().lower().replace("#", "").replace("tag:", "")
                    if cleaned and 2 < len(cleaned) < 30:
    ...
    def _extract_insights(self, content: str) -> List[str]:
        ...
        try:
            prompt = (
                f"Ekstrak 3 insight penting dari konten ini.\n\n"
                f"Setiap insight: 1 kalimat lengkap, maksimal 100 karakter.\n\n"
                f"OUTPUT: HANYA daftar insight, satu per baris, tanpa nomor, tanpa bullet, tanpa pembuka.\n\n"
                f"Contoh output BENAR:\nInterstellar berasal dari bahasa Latin inter + stella.\nVoyager 1 mencapai ruang antarbintang pada 2012.\n\n"
                f"Konten:\n{content[:800]}"
            )
            result = self.ask(prompt, temperature=0.4, max_tokens=250, use_cache=True)
            ...
            for line in result.split(chr(10)):
                ...
                if not line or len(line) < 15:
                    continue
                if any(k in line_lower for k in ["berikut", "insight penting", "contoh output", "daftar insight", "output benar", "catatan:"]):
                    continue
                if line.endswith(":"):
                    continue
                if len(line) > 150:
