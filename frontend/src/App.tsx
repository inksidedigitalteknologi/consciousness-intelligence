...
function AppContent() {
  ...
  const handleUpdateKnowledge = useCallback(
    (id: string, updates: Partial<KnowledgeItem>): void => {
      console.log('🔄 Updating knowledge:', id, updates);
  ...
  const handleDeleteKnowledge = useCallback(
    (id: string): void => {
      console.log('🗑️ Deleting knowledge:', id);
  ...
  const handleBatchEnhance = useCallback(async (): Promise<void> => {
    try {
      const res = await fetch('/api/ai/knowledge/enhance', {
        method: 'POST',
        headers: { 'X-API-Key': API_KEY },
      });
        ...
        setLogs((prev) => [
          {
            ...
            timestamp: Date.now(),
            level: 'SUCCESS',
            message: `Enhanced ${data.enhanced || 0} knowledge items with AI`,
            source: 'Knowledge',
          },
          ...prev.slice(0, 99),
        ]);
        // Refresh knowledge list dari server
        ...
        setLogs((prev) => [
          {
            ...
            timestamp: Date.now(),
            level: 'ERROR',
            message: `Enhance failed: ${data.error || 'unknown'}`,
            source: 'Knowledge',
          },
          ...prev.slice(0, 99),
        ]);
      ...
      setLogs((prev) => [
        {
          ...
          timestamp: Date.now(),
          level: 'ERROR',
          message: `Enhance error: ${err instanceof Error ? err.message : 'unknown'}`,
          source: 'Knowledge',
        },
        ...prev.slice(0, 99),
      ]);
    ...
    <div className="flex flex-col h-screen w-screen bg-[#0B0F14] text-[#E8EDF2] overflow-hidden font-sans">
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto px-2 py-4 sm:px-3 sm:py-4 pb-20 sm:pb-4 scrollbar-thin scrollbar-thumb-[#26313D] scrollbar-track-transparent">
