...
export const KnowledgeView: React.FC<KnowledgeViewProps> = ({
  ...
  // Update tag cloud and category stats
  useEffect(() => {
    ...
    <div id="knowledge-view" className="space-y-6 pb-12">
      ...
      {notification && (
        <div className={`p-3 rounded-xl flex items-center gap-2 text-xs ${
          ...
          notification.type === 'error' ? 'bg-rose-500/10 border border-rose-500/30 text-rose-400' :
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white tracking-wide">
              Knowledge Base & AI Memory v1.02
            </h2>
      ...
      <div className="space-y-3">
        <button
          ...
          className="text-xs font-bold text-[#8D9AAA] hover:text-white flex items-center gap-1"
        >
            ...
            <div className="col-span-2 md:col-span-4 pt-3 border-t border-[#26313D]/50">
              ...
              <div className="flex flex-wrap gap-1.5">
                {Object.entries(tagCloud).sort((a, b) => b[1] - a[1]).slice(0, 15).map(([tag, count]) => (
                  <span
                    key={tag}
                    className="text-[10px] px-2 py-0.5 rounded-full bg-[#1A2530] text-[#8D9AAA] hover:text-white cursor-pointer transition-colors"
                    style={{ fontSize: `${10 + Math.min(count * 2, 20)}px` }}
      ...
      <div className="p-5 rounded-2xl bg-[#1A2530] border border-[#26313D] space-y-3 shadow-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[8px] px-2 py-0.5 rounded bg-[#26313D] text-[#8D9AAA]">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-[9px] px-2 py-0.5 rounded bg-[#0B0F14] hover:bg-[#26313D] text-[#8D9AAA] hover:text-white transition-colors"
            >
        ...
        {qaAnswer && (
          <div className="p-3.5 rounded-xl bg-[#0B0F14] border border-teal-500/30 text-xs text-teal-200 leading-relaxed font-sans whitespace-pre-line">
            <div className="flex items-center gap-2 mb-1">
              ...
              {qaSources.length > 0 && (
                <span className="text-[9px] text-[#5F6B78]">({qaSources.length} sources)</span>
              )}
            </div>
            {qaAnswer}
            {qaSources.length > 0 && (
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {(['text', 'url', 'raw', 'ai'] as const).map((tab) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-bold transition-colors ${activeTab === tab
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => {
                    if (textContent.trim()) {
                      const words = textContent.toLowerCase().split(' ').slice(0, 5);
                      const suggested = words.find(w =>
                        ['trading', 'market', 'finance', 'strategy', 'bitcoin', 'crypto', 'ai', 'economy'].includes(w)
                      ) || 'Trading';
                      setCategory(suggested.charAt(0).toUpperCase() + suggested.slice(1));
                    }
                  }}
      ...
      <div className="p-4 rounded-xl bg-[#131A22] border border-[#26313D] flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className={`text-[10px] px-2 py-1 rounded border ${isBulkMode ? 'bg-teal-600 text-white border-teal-500' : 'bg-[#0B0F14] text-[#8D9AAA] border-[#26313D]'}`}
          >
            ☑ Bulk
          </button>
          <button
            onClick={clearFilters}
            className="text-[10px] px-2 py-1 rounded bg-[#0B0F14] border border-[#26313D] text-[#8D9AAA] hover:text-white"
          >
            ✕ Clear Filters
          </button>
          <button
            onClick={() => setKeyboardMode(!keyboardMode)}
            className={`text-[10px] px-2 py-1 rounded border ${keyboardMode ? 'bg-blue-600 text-white border-blue-500' : 'bg-[#0B0F14] text-[#8D9AAA] border-[#26313D]'}`}
          >
            ⌨️ Shortcuts {keyboardMode ? 'ON' : 'OFF'}
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <select
            ...
            className="px-2 py-1 rounded bg-[#0B0F14] border border-[#26313D] text-xs text-white"
          >
          ...
          <button
            ...
            className="px-2 py-1 rounded bg-[#0B0F14] border border-[#26313D] text-xs text-[#8D9AAA] hover:text-white"
          >
            {sortOrder === 'asc' ? '↑' : '↓'}
          </button>
          <select
          ...
          <button
            ...
            className={`text-[10px] px-2 py-1 rounded border ${showArchived ? 'bg-amber-600 text-white border-amber-500' : 'bg-[#0B0F14] text-[#8D9AAA] border-[#26313D]'}`}
          >
            🗂️ Archived
          </button>
        </div>
        ...
        <div className="p-3 rounded-xl bg-[#1A2530] border border-teal-500/30 flex flex-col sm:flex-row items-center justify-between gap-2">
          <span className="text-xs text-white font-bold">
            ...
            <button
              ...
              className="ml-2 text-[#8D9AAA] hover:text-white text-[10px]"
            >
              {selectedIds.length === filteredKnowledge.length ? 'Deselect All' : 'Select All'}
            </button>
            <button
              onClick={() => setSelectedIds([])}
              className="ml-2 text-[#8D9AAA] hover:text-white"
            >
              ✕
            </button>
          </span>
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1">
              <input
                className="px-2 py-1 rounded-lg bg-[#0B0F14] border border-[#26313D] text-xs text-white placeholder-[#5F6B78] w-24"
                placeholder="Tag"
                value={bulkTagInput}
              ...
              <button
                ...
                className="px-2 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs"
              >
                Add
              </button>
              <button
                onClick={() => handleBulkTag('remove')}
                className="px-2 py-1 rounded-lg bg-rose-600 hover:bg-rose-500 text-white text-xs"
              >
                Remove
              </button>
            </div>
      ...
      <div className="p-4 rounded-xl bg-[#131A22] border border-[#26313D] flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          className="px-2 py-1 rounded-lg bg-[#0B0F14] border border-[#26313D] text-xs text-white"
        >
          {categories.map((c) => (
            <option key={c} value={c}>{c}</option>
          ))}
        ...
        <input
          ...
          className="w-20 px-2 py-1 rounded-lg bg-[#0B0F14] border border-[#26313D] text-xs text-white placeholder-[#5F6B78] focus:outline-none focus:border-teal-500"
        />
        <input
          type="date"
        ...
        <button
          ...
          className="px-2 py-1 rounded-lg bg-[#0B0F14] border border-[#26313D] text-[#8D9AAA] hover:text-white"
        >
          {viewMode === 'grid' ? '⊞' : '≡'}
        </button>
        <span className="text-[10px] text-[#5F6B78] ml-auto">
      ...
      <div className="p-5 rounded-2xl bg-[#131A22] border border-[#26313D] space-y-4 shadow-lg">
        ...
        {isLoading && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="p-4 rounded-xl bg-[#1A2530] border border-[#26313D] animate-pulse h-24" />
            ))}
          </div>
        )}
        ...
        {!isLoading && filteredKnowledge.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-[#8D9AAA] text-sm">
              {searchFilter || selectedCategoryFilter !== 'All' || selectedTypeFilter !== 'All' || tagFilter
        ...
        {!isLoading && filteredKnowledge.length > 0 && (
          <div className={viewMode === 'grid'
            ...
            {filteredKnowledge.map((item) => (
              <div
                key={item.id}
                className={`p-4 rounded-xl bg-[#1A2530] border border-[#26313D] hover:border-teal-500/40 transition-all duration-300 group ${item.ai_enhanced ? 'border-purple-500/20' : ''
                  } ${selectedIds.includes(item.id) ? 'border-teal-500 bg-teal-500/10' : ''}`}
              >
                    ...
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="text-[#8D9AAA] hover:text-white transition-colors"
                          title="Edit"
                        >
                  ...
                  <div className="flex items-center gap-1 flex-wrap">
                    {item.ai_tags && item.ai_tags.length > 0 ? (
                      item.ai_tags.slice(0, 3).map((tg) => (
                        <span
                          key={tg}
                          className="text-purple-400/70 cursor-pointer hover:text-purple-400"
                      ...
                      item.tags.slice(0, 3).map((tg) => (
                        <span
                          key={tg}
                          className="text-[#8D9AAA] cursor-pointer hover:text-white"
                          onClick={() => setTagFilter(tg)}
                        >
                          #{tg}
                        </span>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setEditingItem(null)}>
          <div className="bg-[#1A2530] border border-[#26313D] rounded-2xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setEditingItem(null)} className="text-[#8D9AAA] hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-3">
              <div>
                ...
                <textarea
                  rows={4}
                  className="w-full p-3 rounded-xl bg-[#0B0F14] border border-[#26313D] text-xs text-white focus:outline-none focus:border-teal-500"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    className="w-full p-2 rounded-xl bg-[#0B0F14] border border-[#26313D] text-xs text-white focus:outline-none focus:border-teal-500"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingItem({ ...editingItem, confidence: Number(e.target.value) })}
                    min="0"
                    max="100"
                  />
                </div>
              ...
              <button
                onClick={() => handleSaveEdit(editingItem.id, {
                  content: editingItem.content,
                  category: editingItem.category,
                  tags: editingItem.tags,
                  confidence: editingItem.confidence,
                })}
                className="w-full py-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-white text-xs font-bold transition-colors"
              >
                💾 Save Changes
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowImportModal(false)}>
          <div className="bg-[#1A2530] border border-[#26313D] rounded-2xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowImportModal(false)} className="text-[#8D9AAA] hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-3">
              <div>
                ...
                <textarea
                  rows={6}
                  className="w-full p-3 rounded-xl bg-[#0B0F14] border border-[#26313D] text-xs text-white font-mono focus:outline-none focus:border-teal-500"
                  placeholder='[{"content": "Your knowledge", "category": "Trading", "tags": ["tag1"]}]'
                  value={importData}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowShortcuts(false)}>
          <div className="bg-[#1A2530] border border-[#26313D] rounded-2xl p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowShortcuts(false)} className="text-[#8D9AAA] hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-2 text-xs text-[#8D9AAA]">
              <div className="flex justify-between py-1 border-b border-[#26313D]/40">
                <span>Ctrl+K</span>
      ...
      <div className="text-[9px] text-[#5F6B78] text-center py-2 border-t border-[#26313D]/50 flex flex-wrap items-center justify-center gap-2">
        ...
        <span className="w-1 h-1 rounded-full bg-[#26313D]"></span>
        <span>AI {aiStatus?.enabled ? '🟢 ENABLED' : '🟡 DISABLED'}</span>
        <span className="w-1 h-1 rounded-full bg-[#26313D]"></span>
        <button
