...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0F141B]/95 backdrop-blur-md border-t border-[#26313D] px-2 py-1.5 flex items-center justify-around select-none shadow-2xl safe-area-bottom">
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center justify-center py-1 px-2 rounded-xl transition-all cursor-pointer relative min-w-[56px] min-h-[44px] ${
              isActive ? 'text-blue-400 font-bold' : 'text-[#8D9AAA] hover:text-white'
            }`}
          >
            <div className="relative">
              <Icon className={`w-5 h-5 ${isActive ? 'text-blue-400 scale-110' : ''}`} />
              {item.badge !== undefined && item.badge > 0 && (
                <span className="absolute -top-1.5 -right-2 w-4 h-4 rounded-full bg-amber-500 text-black text-[9px] font-black font-mono flex items-center justify-center">
                  {item.badge}
                </span>
              )}
            </div>
