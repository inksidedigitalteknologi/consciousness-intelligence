...
interface SystemMetrics {
  ...
  prediction_accuracy_note?: string;
...
export const PredictionView: React.FC = () => {
  ...
  return (
    <div id="prediction-view" className="space-y-6 pb-12">
      ...
      {/* METRICS GRID */}
      <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8 gap-3.5">
        <div className="p-4 rounded-xl bg-[#131A22] border border-[#26313D] hover:border-[#3A4A5A] transition-colors">
          <div className="flex items-center gap-2">
            <Target className="w-3.5 h-3.5 text-emerald-400" />
            <span className="text-[10px] uppercase font-bold text-[#8D9AAA]" title="Execution success rate — bukan akurasi prediksi">Execution Rate</span>
          </div>
          <span className="text-xl font-bold font-mono text-emerald-400 mt-1 block">
            {metrics?.execution_success_rate != null ? `${(metrics.execution_success_rate * 100).toFixed(1)}%` : '—'}
          </span>
          <span className="text-[9px] text-[#5F6B78] mt-0.5 block font-mono">
            {metrics?.total_cycles ? `${metrics.successful_cycles}/${metrics.total_cycles} cycles` : `${predictions.length} active forecasts`}
          </span>
        </div>
      ...
      {monteCarlo && (
        <div className="p-5 rounded-2xl bg-[#131A22] border border-[#26313D] space-y-4 shadow-lg animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-3.5 font-mono text-xs">
          ...
          {predictions.length > 0 ? (
            predictions.map((f) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-2 font-mono text-xs pt-1">
